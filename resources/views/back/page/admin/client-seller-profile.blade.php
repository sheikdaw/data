...
<div class="row container d-flex align-items-stretch">
    <div class="col-xl-4 col-lg-4 col-sm-12 col-md-4 mb-30 card border-left-primary shadow">
        <div class="pd-20 card-box height-100-p justify-content-center text-center"> <!-- Added text-center class -->
            <div class="profile-photo">
                <img src="{{ $client->picture }}" alt="" class="avatar-photo rounded-circle" id="clientProfilePicture">
                <a href="javascript:;" class="edit-avatar" onclick="event.preventDefault();document.getElementById('clientProfilePictureFile').click();">
                    <i class="fas fa-pencil-alt"></i>
                </a>
                <input type="file" name="clientProfilePictureFile" id="clientProfilePictureFile" class="form-control-file mt-3 d-none">
            </div>
            <h5 class="text-center h5 mb-0" id="ProfileName">{{ $client->name }}</h5>
            <p class="text-center text-muted font-14" id="ProfileEmail">{{ $client->email }}</p>
    ...
    <div class="col-xl-8 col-lg-8 col-md-8 col-sm-12 mb-30 card border-left-primary shadow">
        <div class="card-box height-100-p overflow-hidden">
            @livewire('client-seller-profile', ['id' => $client->id])
        </div>
    </div>
</div>
...
<script>
   ...
   window.addEventListener('updateInfo', function (event) {
    ...
    $('#clientProfilePictureFile').ijaboCropTool({
          ...
          processUrl:'{{ route("admin.change-client-profile-picture") }}',
       ...
       function updateProfilePicture() {
        var x=  $('#ProfileName').val();
        $.ajax({
            type: 'POST',
            url: '{{ route("admin.get-client-profile-picture") }}',
            data: x =x,
            success: function (response) {
                console.log('Update Profile Picture Response:', response);
                if (response.status === 1 && response.picture) {
                    $('#clientProfilePicture').attr('src', response.picture);
